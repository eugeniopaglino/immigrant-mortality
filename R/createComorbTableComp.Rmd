...
```{r}
files <- sapply(2017:2019,function(x) here(inDir,glue('mort{x}'),glue('MULT{x}USAllCnty.txt')))
...
comorbTable <- comorbRecords %>%
  ...
  filter(majorCauses!='All other causes',
...
comorbTable %>%
  ...
  gtsave(here('figures','comorbTableUCDUSComp.png'),
...
comorbTable %>%
  ...
  gtsave(here('figures','comorbTableUCDForeignComp.png'),
       vwidth = 1000,vheight = 2000,zoom=4)
```

```{r}
comorbTable %>%
  write_csv(here(outDir,'comorbTableUCDComp.csv'))
```
