...
create_contributions_data <- function(data) {
  contributionsData <- data %>%
    ...
    select(-c(deaths,pop,logASDR,ASDR.SD,coefOfVar)) %>%
    pivot_wider(names_from = foreign, values_from = ASDR) %>%
    rename(ASDRForeign = 'Foreign-Born',
           ASDRUSBorn = 'US-Born') %>%
    mutate(ASDRDiff = ASDRUSBorn - ASDRForeign)
...
create_diff_in_diff_data <- function(data) {
  ...
  diffInDiffData <- data %>% 
    pivot_wider(names_from = period, values_from = ASDRForeign:ASDRDiff) %>%
    mutate(ASDRDiffInDiff = (`ASDRForeign_2017-2019` - ASDRForeign_2020) - (`ASDRUSBorn_2017-2019` - ASDRUSBorn_2020))
...
contributions_plot_color_safe <- function(data,thisSex) {
  contributionsPlot <- data %>%
    ...
    labs(x='',
         y='',
         fill='',
         title='Cause-Specific Contributions to the Difference between US-Born and Foreign-Born ASDR') +
    theme_minimal() +
...
diff_in_diff_plot_color_safe <- function(data,thisSex) {
  diffInDiffPlot <- data %>%
    ...
    labs(x='',
         y='',
         fill='',
         title='Changes in the Cause-Specific Contributions to the Difference between US-Born and Foreign-Born ASDR (2020 - 2017-2019)') +
    theme_minimal() +
    theme(legend.position = 'bottom',
          panel.grid.major.y = element_blank(),
          panel.grid.minor.y = element_blank(),
  ...
  contributionsTable <- data %>%
    ...
    cols_label(
      ...
      `ASDRDiff_2020` = 'Difference',
       ASDRDiffInDiff = 'Difference in Differences') %>%
    fmt_number(
...
```{r}
gtsave(contributionsTableMales,here('figures','contributionsTableMales.png'),
       vwidth = 1400,vheight = 1000, zoom=4)
gtsave(contributionsTableFemales,here('figures','contributionsTableFemales.png'),
       vwidth = 1400,vheight = 1000, zoom=4)
```